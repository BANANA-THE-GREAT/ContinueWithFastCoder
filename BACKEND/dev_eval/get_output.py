...
def run_eval(model, tokenizer, max_token_span, num_draft, temperature, top_p, max_new_token):
    ...
    fp=open(f'/home/zhaoqianhui/REST/dev_eval/retrieved_tokens/repo_tokenized.jsonl','a',encoding='utf-8')
        ...
        for repo in dataset[topic]:
            ...
            for sample in tqdm(repo_dataset, total=len(repo_dataset)):
                ...
                with torch.inference_mode():
                    ...
                    for i in range(2000):
                        candidates, tree_candidates, draft_buffers, this_token = generate_candidates_and_draft_buffer(
                                logits,
                                input_ids,
                                datastore,
                                token_spans,
                        ...
                        if accept_length_tree > 1:
                            # 获取新接受的tokens并单独解码
                            retrieved_accepted_tokens = input_ids[0, -accept_length_tree+1:].tolist()
                            decoded_tokens = [tokenizer.decode([token_id], skip_special_tokens=True) for token_id in retrieved_accepted_tokens]
                            
                            # 计算token在生成序列中的位置索引
                            token_start_idx = len(input_ids[0, input_len:-accept_length_tree+1])
                            token_end_idx = len(input_ids[0, input_len:])

                            retrieved_tokens.append({
                                'token_for_retrieval': [tokenizer.decode([token_id], skip_special_tokens=True) for token_id in this_token], # 用于检索的token
                                'retrieved_tokens': decoded_tokens,  # 每个被检索到的token解码后的值列表
                                'start_idx': token_start_idx,  # token在生成序列中的起始位置
                                'end_idx': token_end_idx,      # token在生成序列中的结束位置
                        ...
                        if model.tokenizer.eos_token_id in input_ids[0, input_len:] or new_token > max_new_token:
                            ...
                            full_generated_text = [tokenizer.decode([token_id], skip_special_tokens=True) for token_id in generated_tokens]
                            break
...
if __name__ == "__main__":
    ...
    # dataset=[]
    # f=open("/home/jiaoziqian/LLMAcceleration-REST/dev_eval/LM_prompt_retrieval_communications.jsonl",'r',encoding='utf-8')
    # for l in f.readlines():
