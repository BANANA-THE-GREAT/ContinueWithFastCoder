...
import sys
# sys.path.append("../")
sys.path.append("/home/zhaoqianhui/REST/")
import torch
...
def load_dataset(file):
    ...
    for l in f.readlines():
        data=json.loads(l)
        repo=data['metadata']['task_id'].split('/')[0].replace('--','_')
        if repo not in dataset.keys():
            dataset[repo]=[]
        dataset[repo].append(data)
    return dataset

def run_eval(model, tokenizer, max_token_span, num_draft, temperature, top_p, max_new_token):
    fw=open('0113.txt','a',encoding='utf-8')
    ...
    for repo in dataset:
        repo_dataset=dataset[repo]
        print(colored(f'{repo}',"blue"))
        # load datastore
        # print("loading the datastore ...")
        repo_datastore = draftretriever.Reader(
            index_file_path=f'/home/zhaoqianhui/REST/datastore/repoeval/repo_datastore/datastore_repo_{repo}.idx',
            # index_file_path=f'/home/zhaoqianhui/REST/datastore/repoeval/repoandngram_datastore_cl/datastore_repoandngram_{repo}.idx',
        )

        # ngram_datastore = draftretriever.Reader(
        #     index_file_path=f'/home/zhaoqianhui/REST/datastore/repoeval/repo_ngram_datastore_cl/datastore_ngram_{repo}.idx',
        # )

        # print("datastore loaded!")
        datastores=[stack_datastore, repo_datastore]
        # datastores=[stack_datastore, repo_datastore, ngram_datastore]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/home/zhaoqianhui/REST/repo_eval/RepoCoder/datasets/function_level_completion_2k_context_codex.test.jsonl",
