...
class TestXMLTestCase(XMLTestCase):
    ...
    def test_assertSubtreeEqual_forbid_reordering_of_different(self):
        t1 = etree.fromstring("<foo><bar a='1' /><bar a='2' /></foo>")
        t2 = etree.fromstring("<foo><bar a='2' /><bar a='1' /></foo>")
        
        with self.assertRaises(AssertionError):
            self.subTest("forbid_reordering_of_different", t1, t2)
            
    def test_assertSubtreeEqual_forbid_reordering_of_different_strict(self):
        t1 = etree.fromstring(
            "<foo><bar a='1' /><bar a='2' /><bar a='3' /></foo>")
        t2 = etree.fromstring(
            "<foo><bar a='2' /><bar a='1' /><bar a='3' /></foo>")
        
        with self.assertRaises(AssertionError):
            self.subTest("forbid_reordering_of_different_strict", t1, t2)